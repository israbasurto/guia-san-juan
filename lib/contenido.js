...
import { diasDesde, DIAS_VENCE, DIAS_AVISO_PUBLICO } from './cola-verificacion';
...
export function frescura(rows) {
  ...
  const diasMasViejo = diasDesde(min); // el grupo más antiguo marca el aviso
  return {
    min, max, unaSola: min === max,
    porGrupo,
    vencido: diasMasViejo != null && diasMasViejo > DIAS_VENCE,
    muyViejo: diasMasViejo != null && diasMasViejo > DIAS_AVISO_PUBLICO,
    // etiqueta principal
    etiqueta: min === max
